...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=347,
                 hidden_layers_sizes=[25, 25, 25], n_outs=9):
    ...
    # Function that returns two lists:
...
def test_DBN(finetune_lr=0.1, pretraining_epochs=1000,
             pretrain_lr=0.01, k=1, training_epochs=20000,
    ...
    if dataset == 'pavia_centre':
        datasets, train_set_x, train_set_y, valid_set_x, valid_set_y, test_set_x, test_set_y = load_mat()
    ...
    dbn = DBN(numpy_rng=numpy_rng, n_ins=347,
              hidden_layers_sizes=[25, 25, 25],
              n_outs=9)

    # start-snippet-2
     
    # number of images in the example set
    ...
    extract_prob = theano.function(
            ...
            givens={
                dbn.x: test_set_x[index*0 : len(test_set_x.get_value())]
            }
        )
    ...
    print 'Number of pre-training epochs: %i\n' % pretraining_epochs
        ...
        for epoch in xrange(pretraining_epochs):
            ...
            for batch_index in xrange(n_train_batches):
                c.append(pretraining_fns[i](index=batch_index,
                                            lr=pretrain_lr))
                # print 'Cost of batch %i: %d' % (batch_index, c[batch_index]) 
            print 'Pre-training layer %i, epoch %d, cost ' % (i, epoch),
            print numpy.mean(c)
            out_file = file('output.txt', 'ab')
            out_file.write('Pre-training layer %i, epoch %d, cost ' % (i, epoch))
            out_file.write(numpy.mean(c))
            out_file.close()
    ...
    params_file = file('params_pretrain.pkl', 'wb')
    for i in xrange(4):
    ...
    patience = 5000 * n_train_batches  # look as this many examples regardless
    print 'Number of finetuning epochs: %i\n' % training_epochs
    print 'Patience: %i\n' % patience
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    out_file = file('output.txt', 'ab')
                    out_file.write(('     epoch %i, minibatch %i/%i, test error of '
                           'best model %f %%') %
                          (epoch, minibatch_index + 1, n_train_batches,
                           test_score * 100.))
                    out_file.close()
                    ...
                    params_str = 'params_' + str(epoch) + '.pkl'
                    params_file = file(params_str, 'wb')
                    ...
                    y_prob = extract_prob(0)
                    y_pred = extract_pred(y_prob)                    

                    y_file_str = 'y_file_' + str(epoch) + '.pkl'
                    y_file = file(y_file_str, 'wb')

                    cPickle.dump(y_prob, y_file, protocol = cPickle.HIGHEST_PROTOCOL)
                    cPickle.dump(y_pred, y_file, protocol = cPickle.HIGHEST_PROTOCOL)
                    y_file.close()
    ...
    # Print number of weights/biases in each layer
    for i in xrange(len(final_weights)):
        print(
            (
                'layer: %i, number of weights: %i\nlayer: %i, number of biases: %i'
